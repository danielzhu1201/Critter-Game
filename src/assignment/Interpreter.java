		...
		while (i < commands.size() && i >= 1 && p < 1000) {
			...
			// IfEmpty
				...
				if (bear <= 315 && bear % 45 == 0 && bear>=0) {
					if (c.getCellContent(bear) == Critter.EMPTY) {
			...
			// IfAlly
				...
				int bear = Interpreter.parseInt(bearing);
				if (bear <= 315 && bear % 45 == 0 && bear>=0) {
					if (c.getCellContent(bear) == Critter.ALLY) {
						String instruction = cur.substring(temp);
						i = this.gotostepn(instruction, i);
						continue;
					}
			...
			// IfEnemy
				...
				if (bear <= 315 && bear % 45 == 0) {
					if (c.getCellContent(bear) == Critter.ENEMY) {
						String instruction = cur.substring(temp);
						i = this.gotostepn(instruction, i);
						continue;
					}
			...
			// IfWall
				...
				int bear = Interpreter.parseInt(bearing);
				if (bear <= 315 && bear % 45 == 0) {
					if (c.getCellContent(bear) == Critter.WALL) {
						String instruction = cur.substring(temp);
						i = this.gotostepn(instruction, i);
						continue;
					}
			...
			// infect - 2 situations
				...
				if (cur.length() == 6) {
					...
					if (line == -1000 || line >= commands.size())
						c.setNextCodeLine(commands.size() + 1);
					else {
						c.infect(line);
						c.setNextCodeLine(i + 1);
